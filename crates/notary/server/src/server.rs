...
use crate::{
    ...
    service::{initialize, upgrade_protocol, verify_presentation},
...
pub async fn run_server(config: &NotaryServerProperties) -> Result<(), NotaryServerError> {
    ...
    // let hex_public_key = hex::encode(public_key.as_bytes());
    // info!("Public Key (Hex): {}", hex_public_key);
    ...
    let router = Router::new()
        ...
        // Add for verify json format presentation files generated by web extension
        .route("/verifyPresentation",post(verify_presentation))
